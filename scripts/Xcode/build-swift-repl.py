...
def linker_args_for_repl():
    return [arg for args in map(linker_args_for_rpath, repl_rpaths()) for arg in args]
...
def swiftc_args_for_repl():
    return [swiftc_path(), "-DXCODE_BUILD_ME"] + target_arg_for_repl() + ["-g", "-o", repl_executable(), repl_source_file()] + module_cache_args_for_repl() + linker_args_for_repl()
