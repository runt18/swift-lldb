...
class TestSwiftExpressionScopes(TestBase):
    ...
    def do_test(self):
        ...
        # Set the breakpoints
        shadow_a_bkpt = target.BreakpointCreateBySourceRegex('Shadowed in A', self.main_source_spec)
        self.assertTrue(shadow_a_bkpt.GetNumLocations() > 0, VALID_BREAKPOINT)

        init_bkpt = target.BreakpointCreateBySourceRegex ('In init.', self.main_source_spec)
        ...
        static_bkpt = target.BreakpointCreateBySourceRegex('In class function', self.main_source_spec)
        ...
        # This test fails on Bryce.  The self metatype doesn't generate a valid type.
        self.check_expression("self.return_ten()", "10", use_summary=False)
        static_bkpt.SetEnabled(False)

        # Now run into the setters & getters:
        set_bkpt = target.BreakpointCreateBySourceRegex('In set.', self.main_source_spec)
        ...
        get_bkpt = target.BreakpointCreateBySourceRegex('In get.', self.main_source_spec)
        ...
        deinit_bkpt = target.BreakpointCreateBySourceRegex('In deinit.', self.main_source_spec)
        ...
        str_sub_get_bkpt = target.BreakpointCreateBySourceRegex('In string subscript getter', self.main_source_spec)
        ...
        # Next run into the closure that captures self and make sure that works:
        closure_bkpt = target.BreakpointCreateBySourceRegex('Break here in closure', self.main_source_spec)
