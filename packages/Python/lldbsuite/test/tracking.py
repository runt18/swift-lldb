...
def get_radar_details(id):
    ...
    if global_radar_client == None:
        ...
        global_radar_client.problem_default_fields = ['id','title','assignee','milestone','component','priority','fixOrder','state','substate','resolution','duplicateOfProblemID']
...
def process_xfail(path):
    ...
    try:
        ...
        pass
    xfail_line = content[content.find('expected failure (problem id:')+len(marker):]
